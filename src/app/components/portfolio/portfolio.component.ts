...
import { transition, style, animate, trigger, stagger, query, animateChild } from '@angular/animations';
...
@Component({
  ...
  animations: [
    trigger('list', [transition(':enter',[query('@items', stagger(60, animateChild() ))])]),
    trigger('items', [transition(':enter', [
...
export class PortfolioComponent implements OnInit {
  ...
  constructor(private serviceholder: PortfolioService, private route: ActivatedRoute) { }
  ...
  loadPortfolios(selectedType: string): void {
    this.serviceholder.get().subscribe(
      data => {
